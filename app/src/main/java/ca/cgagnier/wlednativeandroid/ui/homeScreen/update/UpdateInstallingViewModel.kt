...
class UpdateInstallingViewModel @Inject constructor(
    ...
    fun startUpdate(
        ...
        val updateService = IllumidelUpdateService(device, version, cacheDir)
        updateService.determineAsset()
        if (!updateService.couldDetermineAsset()) {
            Log.w(TAG, "Could not find compatible asset for ${device.name}")
            _state.update { previousState ->
                previousState.copy(
                    canDismiss = true,
                    step = UpdateInstallingStep.NoCompatibleVersion,
                    assetName = updateService.assetName
                )
            }
            return
        }
    ...
    private fun downloadAsset(
        ...
        if (updateService.isAssetFileCached()) {
            Log.d(TAG, "asset '${updateService.assetName}' is already downloaded, reusing")
            installUpdate(updateService)
            return@launch
        }

        Log.d(TAG, "Downloading asset '${updateService.assetName}'")
    ...
    private fun installUpdate(updateService: DeviceUpdateService) {
        Log.d(TAG, "Uploading binary '${updateService.assetName}' to device")
        _state.update { previousState ->
            previousState.copy(
                canDismiss = false,
